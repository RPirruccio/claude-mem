...
export function findCursorHooksDir(): string | null {
  const possiblePaths = [
    ...
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'rpirruccio', 'cursor-hooks'),
...
export function findMcpServerPath(): string | null {
  const possiblePaths = [
    // Marketplace install location
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'rpirruccio', 'plugin', 'scripts', 'mcp-server.cjs'),
    // Development/source location (relative to built worker-service.cjs in plugin/scripts/)
...
export function findWorkerServicePath(): string | null {
  const possiblePaths = [
    // Marketplace install location
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'rpirruccio', 'plugin', 'scripts', 'worker-service.cjs'),
    // Development/source location (relative to built worker-service.cjs in plugin/scripts/)
...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  if (!mcpServerPath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/rpirruccio/plugin/scripts/mcp-server.cjs');
    return 1;
  }
...
export async function installCursorHooks(_sourceDir: string, target: CursorInstallTarget): Promise<number> {
  ...
  if (!workerServicePath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/rpirruccio/plugin/scripts/worker-service.cjs');
    return 1;
  }
  ...
  switch (subcommand) {
    case 'install': {
      ...
      if (!cursorHooksDir) {
        ...
        console.error('   Expected at: ~/.claude/plugins/marketplaces/rpirruccio/cursor-hooks/');
        return 1;
      }
